...
import type { Currency, FinanceData, Frequency, NoticeTone, PurchaseGoal, Vault, WishlistItem } from "@/lib/types";
...
export function PlanningView({ data, updateData, accessToken, onNotice }: { data: FinanceData; updateData: (updater: (current: FinanceData) => FinanceData) => void; accessToken?: string; onNotice: (message: string, tone?: NoticeTone) => void }) {
  const { t } = useLocale();
  ...
  return (
    <div className="stack">
      ...
      {tab === "wishlist" && <Wishlist items={data.wishlist} onAdd={openAdd} onCreateGoal={(item) => { updateData((current) => ({ ...current, goals: [...current.goals, refreshGoal({ id: crypto.randomUUID(), name: item.title, priceCents: item.priceCents, savedCents: 0, contributionCents: item.contributionCents, currency: item.currency, frequency: item.frequency })] })); const message = `Meta “${item.title}” criada com sucesso.`; setFeedback(message); onNotice(message, "success"); }} />}
      {tab === "assistant" && <FinancialAssistant data={data} accessToken={accessToken} onNotice={onNotice} />}

      <VaultForm open={modal === "vault"} onClose={closeModal} onSubmit={(vault) => { updateData((current) => ({ ...current, vaults: [...current.vaults, vault] })); onNotice("Cofrinho criado com sucesso.", "success"); closeModal(); }} />
      <GoalForm key={modal === "edit-goal" ? activeGoal?.id : "new-goal"} goal={modal === "edit-goal" ? activeGoal : null} open={modal === "goal" || modal === "edit-goal"} onClose={closeModal} onSubmit={(goal) => { const editing = modal === "edit-goal"; updateData((current) => ({ ...current, goals: editing ? current.goals.map((item) => item.id === goal.id ? goal : item) : [...current.goals, goal] })); const message = editing ? "Meta atualizada com sucesso." : "Meta criada com sucesso."; setFeedback(message); onNotice(message, "success"); closeModal(); }} />
      <Modal open={modal === "delete-goal"} title="Excluir meta?" description="Esta ação remove a meta do seu planejamento." onClose={closeModal}><p className="goal-delete-copy">A meta <strong>{activeGoal?.name}</strong> será excluída. Os outros dados financeiros não serão alterados.</p><div className="form-actions"><button className="button ghost" type="button" onClick={closeModal}>Cancelar</button><button className="button danger" type="button" onClick={() => { if (!activeGoal) return; updateData((current) => ({ ...current, goals: current.goals.filter((item) => item.id !== activeGoal.id) })); setFeedback("Meta excluída."); onNotice("Meta excluída.", "success"); closeModal(); }}><Trash size={18} />Excluir meta</button></div></Modal>
      <WishForm open={modal === "wish"} accessToken={accessToken} onClose={closeModal} onNotice={onNotice} onSubmit={(item) => { updateData((current) => ({ ...current, wishlist: [...current.wishlist, item] })); onNotice("Desejo salvo com sucesso.", "success"); closeModal(); }} />
      <ContributionForm vault={activeVault} open={modal === "contribution"} onClose={closeModal} onSubmit={(cents) => { if (!activeVault) return; updateData((current) => ({ ...current, vaults: current.vaults.map((item) => item.id === activeVault.id ? { ...item, savedCents: item.savedCents + cents, contributions: [...item.contributions, { id: crypto.randomUUID(), amountCents: cents, date: new Date().toISOString().slice(0, 10) }] } : item) })); onNotice("Valor adicionado ao cofrinho.", "success"); closeModal(); }} />
    </div>
  );
}
...
function FinancialAssistant({ data, accessToken, onNotice }: { data: FinanceData; accessToken?: string; onNotice: (message: string, tone?: NoticeTone) => void }) {
  ...
  async function ask(event: FormEvent) {
    ...
    if (accessToken) {
      try {
        ...
        setBusy(false);
        return;
      } catch { onNotice("A IA online está indisponível. Usamos o cálculo local com seus dados.", "info"); }
    }
...
function VaultForm({ open, onClose, onSubmit }: { open: boolean; onClose: () => void; onSubmit: (vault: Vault) => void }) {
  const [name, setName] = useState(""); const [target, setTarget] = useState(""); const [currency, setCurrency] = useState<Currency>(getStoredCurrency());
...
function GoalForm({ goal, open, onClose, onSubmit }: { goal?: PurchaseGoal | null; open: boolean; onClose: () => void; onSubmit: (goal: PurchaseGoal) => void }) {
  ...
  const [name, setName] = useState(goal?.name ?? ""); const [price, setPrice] = useState(goal ? initialMoney(goal.priceCents) : ""); const [saved, setSaved] = useState(goal ? initialMoney(goal.savedCents) : "0"); const [contribution, setContribution] = useState(goal ? initialMoney(goal.contributionCents) : ""); const [currency, setCurrency] = useState<Currency>(goal?.currency ?? getStoredCurrency()); const [frequency, setFrequency] = useState<Frequency>(goal?.frequency ?? "MONTHLY");
...
function WishForm({ open, accessToken, onClose, onNotice, onSubmit }: { open: boolean; accessToken?: string; onClose: () => void; onNotice: (message: string, tone?: NoticeTone) => void; onSubmit: (item: WishlistItem) => void }) {
  const [url, setUrl] = useState(""); const [title, setTitle] = useState(""); const [price, setPrice] = useState(""); const [image, setImage] = useState(""); const [contribution, setContribution] = useState(""); const [currency, setCurrency] = useState<Currency>(getStoredCurrency()); const [frequency, setFrequency] = useState<Frequency>("MONTHLY"); const [loading, setLoading] = useState(false); const [message, setMessage] = useState("");
  async function inspect() { setLoading(true); setMessage(""); try { const response = await fetch("/api/metadata", { method: "POST", headers: { "Content-Type": "application/json", ...(accessToken ? { Authorization: `Bearer ${accessToken}` } : {}) }, body: JSON.stringify({ url }) }); const data = await response.json() as { error?: string; title?: string; image?: string; price?: number; currency?: string }; if (!response.ok) throw new Error(data.error); setTitle(data.title || title); setImage(data.image || ""); if (data.price) setPrice(String(data.price).replace(".", ",")); setCurrency(data.currency === "USD" ? "USD" : "BRL"); setMessage("Informações encontradas. Revise antes de salvar."); onNotice("Informações do produto encontradas. Revise antes de salvar.", "success"); } catch (error) { const detail = error instanceof Error ? error.message : "Preencha as informações manualmente."; setMessage(detail); onNotice(detail, "error"); } finally { setLoading(false); } }
